...
SET_RUNNING_CONFIG_LOCK_PATH = '/var/run/netsu/set_running_config.lock'
...
class Interface(object):
    ...
    def __init__(self):
        ...
        self._config_validator = validation.Validator(build_config_schema())
        if self._debug_mode:
            self._state_validator = validation.Validator(build_state_schema())
...
def build_parameters_schema():
    definitions = {'Parameters': {'type': 'object', 'properties': {}}}
    for update in plugins.get_parameters_definitions_updates():
        update(definitions)
...
def build_config_schema():
    ...
    for update in plugins.get_config_definitions_updates():
        update(definitions)
...
def build_state_schema():
    ...
    for update in plugins.get_state_definitions_updates():
        update(definitions)
