...
class Application(object):
    ...
    def persistent_config(self):
        if flask.request.method == 'GET':
            return JsonResponse(self._core_interface.get_persistent_config())
        elif flask.request.method == 'PUT':
            requested_config = flask.request.get_json()
            try:
    ...
    def running_config(self):
        if flask.request.method == 'GET':
            return JsonResponse(self._core_interface.get_running_config())
        elif flask.request.method == 'PUT':
            requested_config = flask.request.get_json()
            request_parameters = dict(flask.request.headers)
            try:
    ...
    def system_config(self):
        try:
            return JsonResponse(self._core_interface.get_system_config())
        except netsu.core.errors.NetsuCoreError as e:
            return CoreErrorResponse(e)

    def system_state(self):
        try:
            return JsonResponse(self._core_interface.get_system_state())
        except netsu.core.errors.NetsuCoreError as e:
            return CoreErrorResponse(e)
...
class CreatedResponse(JsonResponse):
    ...
    def __init__(self):
        super().__init__({'message': 'Success'}, status=CreatedResponse.STATUS)
...
class CoreErrorResponse(JsonResponse):
    ...
    def __init__(self, core_error_instance):
        super().__init__({'message': core_error_instance.__doc__},
                         status=CoreErrorResponse.STATUS)
